...
class DiaryCardGateway(
    ...
    async def _set_entity_relationships(self, entity: DiaryCard) -> None:
        if entity.targets:
            entity.targets = (
                await self._session.scalars(select(targets_table).where(targets_table.c.id.in_(entity.targets)))
            ).all()
        if entity.emotions:
            entity.emotions = (
                await self._session.scalars(select(emotions_table).where(emotions_table.c.id.in_(entity.emotions)))
            ).all()
        if entity.medicaments:
            entity.medicaments = (
                await self._session.scalars(select(Medicament).where(medicaments_table.c.id.in_(entity.medicaments)))
            ).all()
        if entity.skills:
            entity.skills = (
                await self._session.scalars(select(Skill).where(skills_table.c.id.in_(entity.skills)))
            ).all()
