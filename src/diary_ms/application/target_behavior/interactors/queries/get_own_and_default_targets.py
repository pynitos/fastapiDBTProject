...
from src.diary_ms.application.target_behavior.dto.target_behavior import GetOwnAndDefaultTargetsQuery, OwnTargetDTO
...
class GetOwnAndDefaultTargets(QueryHandler[GetOwnAndDefaultTargetsQuery, list[OwnTargetDTO]]):
    def __init__(
        self,
    ...
    async def __call__(self, query: GetOwnAndDefaultTargetsQuery) -> list[OwnTargetDTO]:
        user_id: UserId = self._id_provider.get_current_user_id()
        meds: list[Target] = await self._db_gateway.get_all(
            user_id=user_id,
            offset=query.pagination.offset,
            limit=query.pagination.limit,
        )
        dtos: list[OwnTargetDTO] = self._mapper.dm_list_to_dto_list(meds)
        return dtos
